...
public static class StorageTxtVis {
	...
	private static string BuilderArrayToString (
		StringBuilder[] builders) {
	...
	private static void DrawSlotConnections<TN> (
		...
		foreach (var link in links) {
			...
			if (link.nodeA.Position.x < link.nodeB.Position.x
			 && link.nodeA.Position.y == link.nodeB.Position.y) {
				builders[row].
					Append(style.GetConnectionStyle(link.Connection));
			}

			// If the connection is bellow of currentSlot.
			if (link.nodeA.Position.x == link.nodeB.Position.x
			 && link.nodeA.Position.y < link.nodeB.Position.y
			 && row + 1 != builders.Length) {
				builders[row + 1].
					Append(style.GetConnectionStyle(link.Connection));
				builders[row + 1].
					Append(' ');
